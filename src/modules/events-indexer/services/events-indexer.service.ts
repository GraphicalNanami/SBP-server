...
export class EventsIndexerService {
  ...
  async indexTwitterPosts(
    ...
    try {
      ...
      const stellarQuery =
        query || 'stellar OR "stellar lumens" OR xlm OR soroban';
      this.logger.log(`Twitter search query: ${stellarQuery}`);

      const response = await this.twitterService.searchRecentPosts(
        stellarQuery,
        maxResults,
      );
      ...
      // Process posts
      const result =
        await this.postProcessingService.processPosts(transformedPosts);

      this.logger.log(
        `Twitter indexing complete: ${response.posts.length} fetched, ${result.processed} processed`,
      );
      ...
      this.logger.error(
        `Error indexing Twitter posts: ${error.message}`,
        error.stack,
      );
  ...
  async indexRedditPosts(
    ...
    this.logger.log(
      `Starting Reddit indexing - r/${subreddit}, max ${maxResults} posts`,
    );
    ...
    try {
      ...
      let posts;
      if (query) {
        posts = await this.redditService.searchSubreddit(
          subreddit,
          query,
          maxResults,
        );
      ...
      this.logger.log(
        `Reddit indexing complete: ${posts.length} fetched, ${result.processed} processed`,
      );
      ...
      this.logger.error(
        `Error indexing Reddit posts: ${error.message}`,
        error.stack,
      );
      return { fetched: 0, processed: 0, skipped: 0, errors: 1 };
    }
  ...
  async runSingleIndexCycle(
    ...
    // Index Twitter posts
    const twitterResult = await this.indexTwitterPosts(
      twitterQuery,
      maxResults,
    );
    ...
    if (hasRedditConfig) {
      ...
      const postsPerSubreddit = Math.floor(
        maxResults / redditSubreddits.length,
      );

      for (const subreddit of redditSubreddits) {
        const result = await this.indexRedditPosts(
          subreddit,
          redditQuery,
          postsPerSubreddit,
        );
    ...
    this.logger.log(
      `Index cycle complete: ${totalProcessed} total posts processed`,
    );
  ...
  async getIndexingStats(hours: number = 24): Promise<{
    ...
    try {
      const recentPosts =
        await this.postProcessingService.getRecentPosts(hours);
